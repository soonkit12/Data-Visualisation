...
var area = d3.area()
    ...
    .y0(height)
    .y1(function(d) { return y(d.Battery_Current); });

var area2 = d3.area()
    .curve(d3.curveMonotoneX)
    .x(function(d) { return x2(d.Timestamp); })
    .y0(height2)
    .y1(function(d) { return y2(d.Battery_Current); });
...
var context = svg.append("g")
  ...
  ydomain = d3.extent(data, function(d) { return d.Battery_Current; });
  y.domain(ydomain);
  x2.domain(x.domain());
  y2.domain(y.domain());
  ...
  focus.append("path")
    ...
    svg.append("text")
        ...
        .text("Battery Current vs Timestamp");
...
function type(d) {
  ...
  d.Battery_Current = +d.Battery_Current;
  return d;
}
...
function transition(newRange) {
 ...
 function xgridlines(){
    return d3.axisBottom(x).ticks(3)
  }
...
var bisectDate = d3.bisector(function(d) { return d.Timestamp; }).right;
 function mousemove() {  
        ...
        focus.select("text.label")
          ...
          .attr("y", y(d.Battery_Current)-20)
          .attr('dy', 12)
          .text(function() { return "Battery Current :"+ d.Battery_Current; });


        focus.selectAll("circle.y")
             .attr("cx", x(d.Timestamp))
             .attr("cy", y(d.Battery_Current));
