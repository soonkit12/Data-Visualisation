var svg = d3.select("svg"),
    ...
    margin2 = {top: 420, right: 20, bottom: 40,  left: 80},
...
var area = d3.area()
    ...
    .y0(height)
    .y1(function(d) { return y(d['Board Temperature(C)']); });

var area2 = d3.area()
    .curve(d3.curveMonotoneX)
    .x(function(d) { return x2(d.Timestamp); })
    .y0(height2)
    .y1(function(d) { return y2(d['Board Temperature(C)']); });
...
dataset= data
  ...
  ydomain = d3.extent(data, function(d) { return d['Board Temperature(C)']; });
  y.domain(ydomain);
  x2.domain(x.domain());
  y2.domain(y.domain());
  ...
  focus.append("path")
    ...
    svg.append("text")
        ...
        .text("Board Temperature vs Timestamp");
  ...
  svg.append("rect")                                     
        ...
        .on("mouseout", function() { focus.select("text.label").style("display","none");
...
function type(d) {
  ...
  d['Board Temperature(C)'] = +d['Board Temperature(C)'];
  return d;
}
 ...
 function mousemove() {  
   ...
   focus.select("text.label").style("display","block");
		...
		focus.select("text.label")
			...
			.attr("y", y(d['Board Temperature(C)'])-20)
			.attr('dy', -2)
			.attr("text-anchor", "middle")
			...
			.attr("y", y(d['Board Temperature(C)'])-20)
			.attr('dy', 12)
			.text(function() { return "Board Temperature :"+ d['Board Temperature(C)']; });

		focus.selectAll("circle.y")
			.attr("cx", x(d.Timestamp))
			.attr("cy", y(d['Board Temperature(C)']));
